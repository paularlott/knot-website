...
resource: https://getknot.dev/docs/plugins/writing-plugins/pages/
sources:
    - resource: https://getknot.dev/docs/plugins/writing-plugins/pages/
status: stable
title: Plugin Pages
...
A **form popup** returns `{title?, fields, submit?, cancel?}` - the same [field contract](forms.md) as form columns. Submit POSTs to the same handler (with `key`): `error` keeps the popup open with `field_errors` painted on the inputs, `ok` closes it, notifies and refreshes.
...
Trusted markup, rendered raw - the full helper-class reference (`kp-*`), theme rules, available globals (Alpine, Chart.js) and refresh semantics are on the [Raw HTML](html.md) page.
...
Handlers are ajax endpoints: the plugin's own pages, another plugin's pages (see [`pluginFetch`](html.md)), or a user with curl all fetch the same URLs, always answered as JSON. A handler may declare its own gate:
...
So the full model, outermost in: the **page** gate decides the page and every handler riding its path; **row** gates are presentation (gated rows vanish with their columns, hiding their handlers); **column** gates decide both the column's visibility and its handler's fetchability; a **handler declaration** replaces the inherited gates wherever it applies. Handler environments: the scriptling standard library, data formats, templating, text processing, `scriptling.ai`, the [`knot.*` libraries](../../scripting.md) as the requesting user, and binary peers as `plugin.<name>` imports. Installed plugins share one plugin pool and one trust domain, so *other* plugins' exposed surfaces are importable as `plugin.<name>` too ([composition](scriptling.md#composition)). Filesystem access is unrestricted — anything the knot process user can reach, the same authority a binary peer has; outbound HTTP via `requests` (the server's DNS servers when configured, no other policy); no `wait_for` and no container/nomad.
