...
resource: https://getknot.dev/docs/configuration/node-selection/
sources:
    - resource: https://getknot.dev/docs/configuration/node-selection/
status: stable
tags:
...
The order can be pinned with `server.local_containers.runtime_pref` — see [Local Containers](local-containers.md):
...
For stopped local-container spaces, you can edit the space and switch the assigned server before starting it. Before first start this is always available; after that it depends on the template's node migration setting. This is the manual migration path in OSS. Automatic failed-node recovery, detecting a failed node and migrating spaces to a live node, requires Knot Pro .
