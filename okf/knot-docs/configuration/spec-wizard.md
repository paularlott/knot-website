...
resource: https://getknot.dev/docs/configuration/spec-wizard/
sources:
    - resource: https://getknot.dev/docs/configuration/spec-wizard/
status: stable
tags:
...
| `volume` | no | Array of mount points the image expects to be persistent (`[[image.volume]]`). Each entry has a `path` (the in-container mount point, e.g. `/home`, `/data`, `/var/lib/mysql`), a `name` (the suggested managed-volume name — include `${{ .space.id }}` so the resolved name is unique per space and won't clash with other spaces' volumes on the same host; the wizard falls back to the last path segment when omitted), an optional `kind` (`bind`, `volume`, or `path`; defaults to `volume` — the backing kind the wizard pre-fills, which the user can still change), and an optional `description`. The wizard pre-fills one storage row per entry when the image is picked. |
