...
## Audit Anomaly Detection 
 ...
 Knot Pro can run anomaly detection over the audit event stream — failed-login bursts per user, credential spraying per source IP, and event sink delivery failures — emitting its own `Anomaly Detected` audit events when a rule fires. Detection works with any `server.audit.routing` (the internal audit store is not required). See [Anomaly Detection](anomaly-detection.md) for rules, configuration and the interaction with audit routing.
