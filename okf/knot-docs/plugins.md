...
resource: https://getknot.dev/docs/plugins/
sources:
    - resource: https://getknot.dev/docs/plugins/
status: stable
tags:
...
A plugin is **a folder** that declares itself in a `[tool.knot]` table, read from one of two places: the [metadata block](scripting.md) of `main.py` (the `# /// script` comments), or a binary peer's handshake - so a folder with only `bin/` + `assets/` and no `main.py` is a complete plugin. Either way the *same* table is parsed the *same* way. Nothing a plugin declares is produced by running plugin code: whether the manifest comes from a `main.py` block or a peer's handshake it is read as static data - never executed - validated at load, and a plugin's handlers run only when a user reaches its pages or tools, in an environment bound to that user. A broken plugin can never affect server startup. The full metadata reference lives in [Writing Plugins](plugins/writing-plugins.md).
...
- [Writing Plugins](plugins/writing-plugins.md) - a section of its own: packaging, the `[tool.knot]` metadata reference, and validation common to every plugin, then authoring [in Scriptling](plugins/writing-plugins/scriptling.md) and [in Go](plugins/writing-plugins/go.md), with [Plugin Pages](plugins/writing-plugins/pages.md) covering the dispatch model
- [Managing Plugins](plugins/managing.md) - the admin inventory, permissions and roles, lifecycle
