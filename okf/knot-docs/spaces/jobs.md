...
resource: https://getknot.dev/docs/spaces/jobs/
sources:
    - resource: https://getknot.dev/docs/spaces/jobs/
status: stable
tags:
...
Combine jobs with [space schedules](managing.md) (start/stop between hours): a space that auto-starts each workday brings its agent — and therefore its scheduled jobs — up with it automatically.
