...
resource: https://getknot.dev/docs/spaces/log-sinks/
sources:
    - resource: https://getknot.dev/docs/spaces/log-sinks/
status: stable
tags:
...
- **Tunnels included.** A user's sinks also receive the logs of requests proxied through that user's [web tunnels](../configuration/logging.md#tunnel-request-logging) — tagged with the tunnel name instead of a space — plus the tunnels opening and closing (`tunnel_open` / `tunnel_close` records), automatically, no extra configuration. Records carry `service: tunnel` so they select exactly like space records.
...
- **Retry, no disk buffering.** Batches and retries on failure, but if the local service is down for long, records are dropped. This is a developer convenience, not a compliance channel — for that, use [`forward_space_logs`](../configuration/logging.md) on the server (also Pro).
...
The format must match what your log service accepts on that port — the same dialects the agent itself accepts logs in ([see sending logs](logging.md)).
...
Log sinks are a **developer helper, not a compliance channel**. Delivery is best-effort: bounded buffers, no disk spool, drops when the local service is down, and no replay of records missed while a sink was absent. For compliance-grade log retention use the server-side [`forward_space_logs`](../configuration/logging.md) option (Pro) into an external logging service instead.
