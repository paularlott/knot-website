...
resource: https://getknot.dev/docs/spaces/methods/
sources:
    - resource: https://getknot.dev/docs/spaces/methods/
status: stable
tags:
...
See [JSON-RPC Sinks](../../knot-reference/events/json-rpc-sinks.md) for sink configuration details, scope rules, and delivery semantics.
...
`type` defaults to `"stdio"` (the only currently supported transport) and is optional. `mode` is `"concurrent"` (default) or `"serial"`. See [Concurrency Mode](#concurrency-mode) below.
...
Constraint kwargs are snake_case in the builder and emitted as JSON Schema's camelCase equivalents (`min_length` → `minLength`, `additional_properties` → `additionalProperties`, and so on). If a JSON Schema keyword isn't in the curated list below, use `extra={"keyword": value}`.
