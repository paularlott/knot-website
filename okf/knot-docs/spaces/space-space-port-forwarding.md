...
resource: https://getknot.dev/docs/spaces/space-space-port-forwarding/
sources:
    - resource: https://getknot.dev/docs/spaces/space-space-port-forwarding/
status: stable
tags:
...
type: Guide
---
# Space-to-Space Port Forwarding
...
- **direct** : traffic flows directly between agents without going through the server
...
Or use the web UI : open the space's port forwards panel, click edit on a forward, and set latency, jitter, and bandwidth values. Empty fields mean no limit.
