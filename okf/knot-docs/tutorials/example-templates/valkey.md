...
resource: https://getknot.dev/docs/tutorials/example-templates/valkey/
sources:
    - resource: https://getknot.dev/docs/tutorials/example-templates/valkey/
status: stable
tags:
...
The space exports the **Valkey port 6379** via the TCP proxy, allowing it to be connected to using [port forwarding](../../spaces/port-forwarding.md) within the desktop client. For example:
...
As with the Nomad version, you can connect a local port to the Valkey server running within the space using [port forwarding](../../spaces/port-forwarding.md). For example:
