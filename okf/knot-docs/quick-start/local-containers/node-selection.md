...
resource: https://getknot.dev/docs/quick-start/local-containers/node-selection/
sources:
    - resource: https://getknot.dev/docs/quick-start/local-containers/node-selection/
status: stable
tags:
...
2. **Zone Affinity** (optional)
   ...
   - See [Multi-Server Setup](multi-server.md) for zone configuration
...
A stopped local-container space can be reassigned to a different server before it starts. After it has run once, OSS only offers manual stop-and-reassign when the template allows node migration. Automatic failed-node recovery is available in Knot Pro .
