...
resource: https://getknot.dev/docs/templates/managing/
sources:
    - resource: https://getknot.dev/docs/templates/managing/
status: stable
tags:
...
- **`Allow stopped spaces to be migrated between nodes`**:
  Available only for local-container templates (`Local Container`, `Docker`, `Podman`, and `Apple`). When enabled, a stopped space created from the template can be reassigned to a different live node from the edit space form. In Knot Pro , combining node migration with auto-restart on failure enables automatic recovery from failed nodes.
...
- **`Custom Fields`**:
  ...
  - **Autocomplete**: a searchable box whose suggestions come from a [plugin field handler](../plugins/writing-plugins/fields.md) (offered only when a plugin provides one); the value must be one of the suggested options.
...
- [Nomad Templates](nomad-templates.md)
- [Local Container Templates](local-containers.md)
