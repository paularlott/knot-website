...
resource: https://getknot.dev/docs/variables/system-variables/
sources:
    - resource: https://getknot.dev/docs/variables/system-variables/
status: stable
tags:
...
|              | `server.base_image_registry` | The configured base image registry prefix (default `docker.io/paularlott`). Used by the [template spec wizard](../configuration/spec-wizard.md) to prefix manifest entries, and available to any job/volume template that wants to reference the same registry. |
...
When a space is part of a [stack](../stacks.md), it can reference variables belonging to any **sibling space** in the same stack via the `.stack` group. This lets one space consume values produced by another — for example, a web space reading a database password defined on a `db` space.
