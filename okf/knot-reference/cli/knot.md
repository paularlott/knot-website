...
resource: https://getknot.dev/reference/cli/knot/
sources:
    - resource: https://getknot.dev/reference/cli/knot/
status: stable
tags:
...
For commands that run *inside* a space, see the [knot agent](agent.md) reference.
...
`knot stack create` creates one stopped space per component, names each with the given prefix, applies dependencies/port-forwards/custom fields, and groups them under the stack name. See the [knot.stack library reference](../libraries/stack.md) for the scripting API.
