...
resource: https://getknot.dev/reference/events/script-sinks/
sources:
    - resource: https://getknot.dev/reference/events/script-sinks/
status: stable
tags:
...
Scripts are authored on the existing [Scripts](../../knot-docs/scripting.md) page and referenced from the sink by name. This keeps them reusable — one script can serve many sinks.
...
`knot.event.custom()` returns a dict of [custom fields](../../knot-docs/variables/custom-variables.md) from the source space's template. Each key is the field name defined on the template, and the value is what the user entered at space-creation time:
...
Sink scripts run on the knot server in the restricted Server environment — the same library set as MCP tool scripts. That means all `knot.*` API libraries plus the base scriptling libraries and on-demand user `lib` imports, but no system access (`os`, `subprocess`, `pathlib`, …), no `knot.methods`, no `scriptling.runtime`, and `knot.event` exposes the sink accessors only — `emit()` is not registered, which prevents sink → event → sink recursion. See the [Scripting](../../knot-docs/scripting.md) documentation for the full availability tables.
