...
Sink scripts run on the knot server with the full `knot.*` library set (except `knot.event.emit`, which is not registered). See the [Scripting](../../knot-docs/scripting.md) documentation for available libraries.
