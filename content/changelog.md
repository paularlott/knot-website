...
- **Native VictoriaLogs endpoint on the agent**: `POST /insert/jsonline` on the agent's localhost API accepts VictoriaLogs jsonline inserts, honouring the standard `_msg_field` / `_time_field` / `_stream_fields` query parameters — shippers configured for VictoriaLogs work unchanged when pointed at the agent. Extra fields on each line are structured data: they ride along through the agent and server to log sinks (as flat fields, with `service` and `level` declared as streams) and to space log forwarding (as top-level record keys). The same applies to GELF additional fields (`_`-prefixed) and Loki JSON log lines with a `msg`/`message` field. The agent already spoke the Loki and GELF protocols. See [Logging from Spaces](../docs/spaces/logging/).
...
- **On-disk log spooling** {{< pro-badge >}}: undeliverable log batches are spooled to disk while the external logging service is unreachable and replayed oldest-first once it recovers — no lost records during outages. Bounded by size (`max_mb`, default 256 MB) and file count (`max_files`, default 1024 batches) with oldest-first eviction; on recovery up to 32 batches are replayed per flush cycle so backlogs clear quickly. Configured via `[log.spool]` (`enabled`, `path`, `max_mb`, `max_files`).
...
- **Configurable, cluster-wide failed-login blocking**: `auth_ip_rate_limiting` now uses an understandable failure-counter model — after `auth_rate_limit_attempts` failed logins (default 10) within `auth_rate_limit_window` seconds (default 60), auth from that IP / for that account is blocked for `auth_rate_limit_block` seconds (default 300). All three are flags / `server.auth_rate_limit_*` config keys and wizard fields. Failures, blocks and successful-login resets are gossiped between cluster members (in and out of zone), so an attacker spreading attempts across servers behind a load balancer still trips one shared budget and the block applies everywhere. Leaf nodes are not part of the gossip mesh and keep purely local state. Successful logins still reset the counters, and the email limiter is now gated by the enable flag.

- **Config wizard logging step + new options**: a new wizard page configures external log output (URL, format, basic auth or bearer token credentials), audit routing and retention; the Pro wizard also configures space log forwarding {{< pro-badge >}}, the log spool {{< pro-badge >}} and audit anomaly detection {{< pro-badge >}}. The wizard also gained a **cluster section** for cluster deployments (advertise address, cluster key, peers, leaf nodes) on the server-address step, and a **login rate limiting** toggle on the security step — plus a visual refresh aligning it with the main app (numbered step progress, consistent fields, keyboard advancement with Enter).
