...
- **Configurable failed-login blocking**: `auth_ip_rate_limiting` now uses an understandable failure-counter model — after `auth_rate_limit_attempts` failed logins (default 10) within `auth_rate_limit_window` seconds (default 60), auth from that IP / for that account is blocked for `auth_rate_limit_block` seconds (default 300). All three are flags / `server.auth_rate_limit_*` config keys and wizard fields. The email limiter is now also gated by the enable flag, and successful logins still reset the counters.
