...
- **Space log forwarding** {{< pro-badge >}}: logs from spaces now flow into the server's log output, reaching the same destination as the server's own logs (incl. external log services). Delivered as a single copy in multi-server zones. Off by default. See [Logging Configuration](../docs/configuration/logging/).
...
- **On-disk log spooling** {{< pro-badge >}}: undeliverable log batches are spooled to disk and replayed once the external log service recovers — no records lost during outages (bounded at 256 MB by default).
...
- **Config wizard**: new logging step (output URL/format/credentials, audit routing), cluster section for cluster deployments, login-rate-limiting toggle — plus a visual refresh.
