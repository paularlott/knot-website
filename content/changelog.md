...
- **Plugins**: extend knot entirely from a script's metadata — menus, permission-gated pages, themed logos, SVG icons (menus, pages and data-driven row actions all ship their own assets), and Go/scriptling binary peers (peers can carry their assets inside themselves and serve them from a fetcher — single-file plugins, Go binary or scriptling script) — plus a layout-driven page system: handlers return rows of columns, each column declaring its type, data handler and refresh, fetched independently with loaders and per-panel gates; forms are one handler branching on request.method and answer with success/error envelopes (notifications, per-field errors, reset on success), with fields from text and select to an Ace-edited `textarea` with language highlighting; tables carry row actions (icon buttons, kebab menus, confirms in knot's own delete-dialog look, popup forms and information dialogs driven by success envelopes); **field handlers** that turn template custom fields into plugin-backed autocompleters and code editors; plugin handlers exposed as **MCP tools**; and declared **export modules** (`export`) whose classes appear in user tools as `plugin.<name>` — client SDKs over the gated loopback, ergonomics without authority. Plugins declare their system generation (`api = 1`, the default) so a future api 2 can coexist. Plugins load as pure parsing, so a broken one never affects startup. See [Plugins](../docs/plugins/).
...
- **Plugin configuration**: a plugin's deployment settings live in the server's `knot.toml` as a `[plugins.<name>]` table and reach every handler call as `request["config"]` — an inert dict, always present (empty when unconfigured), per-call converted so handlers can never mutate shared state, and identical for pages, MCP tools, field handlers and `knot.plugin.call`. A plugin declares the keys it requires with `[tool.knot] config = [...]`; missing keys fail the plugin at load with a reason on the Plugins page, as does a table over 64 KiB, while a section naming no loaded plugin warns as stale config. See [Plugin configuration](../docs/plugins/writing-plugins/scriptling/#plugin-configuration).

- **Tunnels to any server from a space**: `knot tunnel` inside a space can now target any knot server, not just the one that owns the space — pass `--server` and `--token` (or `KNOT_SERVER` / `KNOT_TOKEN`), or an `-a` alias resolved from the space's `knot.toml` (the same `client.connection.<alias>` layout `knot connect` writes on the desktop), with or without `--daemon`. Several tunnels against different servers and ports can run side by side; each tunnel address is built from your username on the target server and counts against its tunnel quota, and its URL's domain shows which server it runs on. `knot space tunnel` from the desktop gets the same via `--tunnel-server` / `--tunnel-token` / `--tunnel-alias` (also accepted as synonyms on `knot tunnel` in a space), and `knot.space.tunnel_start()` gains optional `server` / `token` arguments. Without an explicit target the tunnel uses the space's own server exactly as before. See [Agent Tunnels](../docs/tunnels/agent-tunnels/).
{{< /changelog-item >}}
