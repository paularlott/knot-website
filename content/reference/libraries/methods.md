...
|-------------|-----------|
| Remote/space scripts (startup scripts), `knot methods register`, `knot run-script` (including server modes) | Available (agent-side). The agent installs the registrar that `register()` calls to publish methods to the server. |
