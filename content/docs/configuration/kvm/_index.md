...
No host setup: the VM attaches to libvirt's `default` NAT network (or any libvirt network you name), DHCPs its address, and reaches the world through the node. Choose it per template with `mode: nat` in the [VM specification](/docs/templates/kvm-templates/vm-spec/).
